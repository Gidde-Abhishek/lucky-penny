...
/**
 * Lucky Penny Stop hook.
 * Triggers session summary generation in the memory worker.
...
async function run(input) {
  ...
  try {
    await fetch(`http://127.0.0.1:${LUCKY_PENNY_MEM_PORT}/api/sessions/summarize`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ claudeSessionId: input.session_id }),
